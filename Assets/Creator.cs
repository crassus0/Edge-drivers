...
public class Creator : MonoBehaviour
{
    ...
    public void LoadLevel(string levelName, string portalName = "")
    {
        ...
        if (x.mainTexture.Length > 0)
            Camera.main.GetComponent<CameraControls>().BackgroundTexture.GetComponent<Renderer>().material.mainTexture = Resources.Load("Background/" + x.mainTexture, typeof(Texture2D)) as Texture2D;
        SceneName = x.name;
        AddObject(m_player);
